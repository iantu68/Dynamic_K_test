...
# 读取数据文件
for i in range(8):
    values1 = np.load(f"expert_grads_L1_FFN0_{i}_nabs.npy", allow_pickle=True)
    values2 = np.load(f"expert_grads_L1_FFN1_{i}_nabs.npy", allow_pickle=True)
    print(values1)
    # # 計算斜率（兩兩值之間的斜率）
    ...
    plt.plot(values1, label='L1_FFN0_na', color='red', linestyle='--')
    # plt.plot(values6, label='L1_a', color='red', linestyle='-')
    plt.plot(values2, label='L1_FFN1_na', color='blue', linestyle='--')
