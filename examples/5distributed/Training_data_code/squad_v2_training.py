...
def train_Bert_MoE(**kwargs):
        ...
        for example in tqdm(examples):
            ...
            if len(answers) > 0:
                ...
                predicted_answers.append(
                    {"id": example_id, "prediction_text": best_answer["text"], "no_answer_probability": 0.0}  # Add default no_answer_probability
                )
            else:
                predicted_answers.append({"id": example_id, "prediction_text": "", "no_answer_probability": 1.0})  # Add default no_answer_probability
    ...
    def preprocess_training_examples(examples):
        ...
        for i, offset in enumerate(offset_mapping):
            ...
            if len(answer["answer_start"]) == 0:  # 無答案的情況
        ...
        # 確保所有字段的長度一致
        assert len(inputs["input_ids"]) == len(inputs["start_positions"]) == len(inputs["end_positions"]), \
            f"Length mismatch: input_ids={len(inputs['input_ids'])}, start_positions={len(inputs['start_positions'])}, end_positions={len(inputs['end_positions'])}"
    ...
    # metric = evaluate.load("squad_v2" if data_args.version_2_with_negative else "squad_v2")
